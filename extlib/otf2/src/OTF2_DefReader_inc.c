...
otf2_def_reader_read_mapping_table( OTF2_DefReader* reader )
{
    ...
    if ( NULL == reader->evt_reader )
    {
        ret = otf2_archive_get_evt_reader( reader->archive,
                                           reader->location_id,
                                           &reader->evt_reader );
        if ( OTF2_SUCCESS != ret )
        {
            OTF2_IdMap_Free( ( OTF2_IdMap* )static_record.id_map );
            return UTILS_ERROR( ret, "Can't get local event reader" );
        }
    }
    ret = otf2_evt_reader_set_mapping_table( reader->evt_reader,
...
otf2_def_reader_read_clock_offset( OTF2_DefReader* reader )
{
    ...
    ret = otf2_evt_reader_add_clock_offset( reader->evt_reader,
                                            static_record.time,
    ...
    if ( OTF2_SUCCESS != ret )
    {
        return UTILS_ERROR( ret, "Can't add ClockOffset defintion to associated event reader." );
    }
