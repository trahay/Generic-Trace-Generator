/*
 * Small test of the state generation in parallel (for ViTE and OTF format)
...
int main (int argc, char** argv){
    ...
    if (argc<2){
        fprintf (stderr, "Usage : mpirun -n 6 ./testState <trace type>. \n 1 for vite, 2 for otf. \n");
        exit (-1);
    }
    MPI_Init (&argc, &argv);
    ...
    CHECK_RETURN (initTrace ("testState_parall", rank));
    if (rank==0){
        /* Creating types used */
